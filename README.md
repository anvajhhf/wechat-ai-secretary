...
- 本地提醒以 `(task_id, reminder_at)` 去重。断网或休眠后会补发；超过 2 小时的多条过期提醒合并成摘要，避免集中轰炸。只有能够证明消息尚未提交发送时才自动重试；发送超时、连接中断或结果不明会标记为待人工确认并停止自动重试，避免重复提醒。
