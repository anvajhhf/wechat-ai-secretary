...
- 非私密语音只在本机转写；中文推荐 Paraformer INT8，通过固定版本 `sherpa-onnx` 在 CPU 上运行。原始音频和转写步骤都不发送给 DeepSeek。转写得到的最小必要文字再按任务/笔记规则分流。
- 语音运行时仅加载项目缓存，缓存缺失或不完整时明确报错，不下载模型、不回退云端。识别器按档案实例串行运行，不读取聊天历史，也不修改进程环境来传递单条消息参数。
...
- 不要求固定前缀：`明天下午3点提醒我回电话`、`记得周五提交报告`会路由为任务；`帮我记一下……`、`整理成笔记……`会路由为笔记。高置信度提醒优先由本地直接处理；若固定语法没有覆盖、但原话仍包含未引用且直接对秘书说出的“提醒我”，只调用一次紧凑的任务专用模型辅助理解，再由本地从原话重新核对事项和时间。模型不能凭普通陈述自行授权写入；转发、引用、提醒状态和“为什么会提醒/你会提醒吗”等询问仍不会创建任务。
- 提醒事项中的“查一下有哪些”“问导师交了吗”“取消会议提醒”等是将来要做的内容，不作为此刻的查询或取消命令。中文钟点与数字钟点等价；时间语境中的 `8.30` 可按 `08:30` 识别，但日期 `2026.09.01` 和数值 `1.50升`不会被误判为钟点。“四点多”仍会追问准确钟点，但不会丢掉已经提供的日期和事项。补充“买牛奶”“下午三点”“不是今天，是明天”时，沿用当前会话中尚未完成的提醒草稿，不要求重说整句。
...
- 语音把“明天”识别成“每天”并进入确认草稿时，可以说“不是每天，是明天”“就明天一次”“明天提醒我一次”，保留原事项及未修改的时间，只替换明确纠正的字段。单独回复“明天”不会擅自取消真正的重复要求；取消、问句、引用和包含新事项的整句也不按旧草稿的单次纠正执行。
- 重复频率和次数只从外层提醒指令提取，事项中“统计共三次报价”等内容不会生成重复提醒。模型自行补出的提醒日期、频率或次数不会写回澄清草稿。
- 单条无文字说明的非私密语音，在新提醒需要澄清时会显示本机实际听到的内容（最多80字），方便用短句纠正。不会将“每天”直接替换成“明天”；微信自身显示的转写和秘书本地识别结果可能不同。
...
- “收到”不会取消提醒。文字消息支持“每天早上8.30、晚上7点提醒我喝中药”这类每日规则，一条任务可有 1—8 个明确且不重复的钟点；本地队列每次送达后只滚动生成该钟点的下一次提醒，不预生成无限记录。有限每周提醒仍支持“每周二上午9点，共3次”，限制为每周一次、共 2 到 52 次；缺少必要字段时只追问缺失项。为防本地语音把“明天”听成“每天”，语音中的每日规则会先回显实际转写并要求确认；每月、每年、复杂工作日规则和其它无限循环仍会明确说明暂不支持。
- 最近唯一提醒可用“改成明天下午四点”调整；“取消刚才那个提醒”只取消本地提醒，不完成或删除滴答任务。取消每日或有限重复提醒时会区分“本次”和“整个系列”：取消本次会跳过最早一次并保留后续每日链，取消整个系列会关闭所有钟点；已提交微信发送的消息无法承诺撤回，发送结果不明时停止当前发送重试，但每日链会从下一个未来钟点继续。
...
```powershell
# 下载并校验所选后端；只准备文件，不切换配置或启动网关
.\scripts\prepare-asr-models.ps1 -Backend paraformer
# 这两个命令配置 Hermes 的离线兼容参数；秘书实际后端由 TOML 选择
...
Paraformer 模型约223 MB，SenseVoice 约239 MB；只用中文推荐先准备 Paraformer。`-Backend both` 两套共约463 MB，首次加装两项固定版本运行组件的下载量约19 MB，不需要 PyTorch/CUDA 大包。模型准备脚本固定源版本、文件大小与哈希，支持分段续传；可显式选 `-ModelHost hf-mirror.com`，但镜像不能绕过同样的校验。它不上传音频、不修改配置、不覆盖校验失败的旧文件。缓存完整时不重复下载。旧 Whisper 缓存保留，不因升级自动删除。

在各自的 `config/secretary.toml` / `config/secretary.partner.toml` 的 `[media]` 内选择 `asr_backend = "paraformer"`、`asr_threads = 2`、`asr_language = "zh"`，备份并逐档案重启后生效。可手动选 `sensevoice` 或 `whisper`；缺少 `asr_backend` 的旧配置仍按 Whisper 处理，不能只看安装成功就认为已切换。网关就绪日志会记录后端名，不记录语音正文。ONNX 文件在 `runtime/models/speech/<backend>/`，每次实例加载前核验，不允许运行时自动下载或回退云端。

ONNX 后端使用16kHz单声道真实重采样、CPU 1—4线程和本地 Silero 人声检测；不裁掉原始前缀，不加热词或提示词，每条新建识别流，最多接受两分钟音频，不重试或调用第二个模型。空结果、无语音信号、异常结果和私密前缀直接停止。SenseVoice 的原生无语音元数据也参与检查，不伪造 Whisper 置信度。

仅保留为手动选项的 Whisper 使用 `asr_model` 和 `asr_vocabulary`。其静态短词表不包含日期猜测或聊天历史，`[]` 可关闭；不超过4秒或无法读取时长的音频不加词表；较长语音出现明确乱码、异常重复时最多去掉词表恢复一次。静音、私密标记、后端异常和超长转写不触发恢复。这些 Whisper 参数不被传给 Paraformer/SenseVoice。

## 模型与上下文成本

- 明确的单项提醒（含文字每日多时段和有限每周提醒）、缺项补充、最近提醒改期/取消/追加和重复消息由本地规则处理，不调用 DeepSeek；同样经过完整的意图、时间、权限和幂等校验。更换本地语音后端不会增加理解模型调用。
- 本地语法未覆盖、但有可靠直接提醒标记的复杂表达最多调用一次任务专用模型，使用比完整自动分类更小的结构；模型结果随后从原话重新落地校验。包含图片、链接、分类、截止时间或多个事项等复杂输入仍保留相应模型处理，不以省 Token 为由丢字段或猜测时间。带多个独立提醒的消息会要求分开处理，不能合并为只提醒一次的任务。
...
- 语音模型全在本地运行，不使用 DeepSeek Token；ONNX 每条只识别一次，Whisper 的有限质量恢复也不调用云端。只有需要模型理解的非私密转写文字才进入 Flash/Pro。事先用文字标记的私密语音不做转写；仅在口述中识别到私密前缀时，立即停止后续外发和任务、笔记写入。
...
后台网关固定以非交互 OAuth 模式运行：滴答令牌失效或刷新失败时，只会停用该连接并写入不含凭证的故障日志，不会自行打开浏览器、重复弹出同意页，也不会替用户点击授权。只有人在终端显式运行上面的 `-AuthorizeDida` 命令时才允许打开官方授权页。
...
提醒与待办按意图分流：明确说“某天某时提醒我……”时只写本地微信提醒队列，不额外创建一条滴答任务；“明天提交报告”“这周把医保报销办了”这类有日期的长期事项仍创建滴答待办，并分别采用配置中公开的默认提醒时间（当前为次日 09:00、整周事项周五 16:00）；“记着买血压计”这类没有日期的事项只进待办，不凭空猜提醒时间。若滴答连接临时故障，已经完全确定时间的本地提醒仍会保存并按时发送，回复同时如实说明滴答写入失败。
...
- 后台日志出现滴答 `401` 或令牌刷新失败时，网关会静默停用该 MCP 连接，不会弹出授权页；要恢复滴答读写，由用户在方便时针对正确档案显式运行 `setup-auth.ps1 -AuthorizeDida`。明确到点的本地微信提醒不依赖这次恢复。
